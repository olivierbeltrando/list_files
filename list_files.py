...
def add_file_to_list(_list, _filename, _parent_directory, _size, _location_label):
    # replacing the ';' by '-' because of csv files

    #filename = discard_invalid_chars(_filename)
    filename = _filename
    if filename != _filename :
        print ('warning ! this file has not its original name : %s' % (filename))
    _list.append([filename.replace(';', '-'), _parent_directory, filename.split('.')[-1], '%.2f Mo' % (_size/(1024*1024)), _location_label])
    ...
    if _simpleSearch :
        ...
        for name in os.listdir(dir_to_search):
            ...
            if os.path.isfile(full_filename):
                ...
                add_file_to_list(ret_list, name, dir_to_search, size, _location_label)
...
def write_cvs(_files, _output_file):
    ...
    encofing = 'utf-8'
    #but this tool is made for windows mainly in french language -> iso 8859-15
